...
function create() {
    ...
    // Handle clicks outside interactive areas
    this.input.on('pointerdown', (pointer) => {
        let clickedInsideArea = false;
        areas.forEach((area) => {
            if (Phaser.Geom.Rectangle.ContainsPoint(new Phaser.Geom.Rectangle(area.x, area.y, area.width, area.height), pointer)) {
...
function displayHappyKid(scene, x, y) {
    const happyKid = scene.add.image(x, y, 'happyKid');
    happyKid.setScale(0.5);
    setTimeout(() => {
        happyKid.destroy();
    }, 1000);
}

function removePanda() {
    if (pandasRemaining > 0) {
        pandas[pandasRemaining - 1].destroy();
        pandasRemaining--;
        if (pandasRemaining === 0) {
            setTimeout(() => {
                Swal.fire({
                    title: 'Game Over!',
                    text: 'You lost all pandas!',
                    icon: 'error',
                    confirmButtonText: 'Restart'
                }).then(() => {
                    restartGame();
...
// Simulate wrong clicks to remove pandas
setTimeout(() => {
    removePanda();
}, 2000);
